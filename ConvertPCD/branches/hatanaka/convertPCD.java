	...
	public static void main(String[] args) {
		try {
			...
			for (int i = 0; i < panoList.getLength(); i++) {		
		    	...
		    	String change[][] = new String[36][5]; //切替情報保持用(オブジェクトの方が良いかも？)
		    	double dist_base = Double.parseDouble(arDist[1][1]); //基準となる距離(最短)
		    	double dist_threshold = dist_base * 3;
		    	int count=0;
		    	for(int j = 1; j<arDist.length; j++){ // 現在のパノラマ画像も含むため j=1 から
		    		if(dist_threshold > Double.parseDouble(arDist[j][1])){ //遠いパノラマ画像の近傍情報付加を行わないため
		    			if((arLink[(int)Math.floor(Double.parseDouble(arDist[j][2]))] == null)){
		    				// 切替範囲の割当計算(要変更)
		    				double dist_ratio = Double.parseDouble(arDist[j][1]) / dist_base;
		    				double tmp = Math.pow(0.80, dist_ratio) * 60 + 12;　　//最大60°、距離が2倍だと約50°、3倍だと約42°
